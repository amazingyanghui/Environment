...
;layui.extend({setter: "config", admin: "lib/admin", view: "lib/viewnew"}).define(["setter", "admin"], function (e) {
    var a = layui.setter, n = layui.element, i = layui.admin, t = i.tabsPage, l = layui.view, o = function () {
        ...
        if (layui.each(a.indPage, function (e, a) {
                if (d === a) return n = !0
            }), layui.config({base: a.base + "controller/"}), n || "/user/nologin" === d) r.render(t.path.join("/")).done(function () {
            i.pageType = "alone"
        }); else {
            if (a.interceptor) {
                var u = layui.data(a.tableName);
                if (!u[a.request.tokenName]) return location.hash = "/user/nologin/redirect=" + encodeURIComponent(d);
            }
            "console" === i.pageType ? o() : r.render("layoutnew").done(function () {
                o(), layui.element.render(), i.screen() < 2 && i.sideFlexible(), i.pageType = "console"
            })
        }
    }, d = "#LAY_app_body", u = "layadmin-layout-tabs", s = layui.$, c = s(window);
