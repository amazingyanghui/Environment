layui.define(['jquery', "formSelects",'quarterlyCycle'], function (exports) {
    // var $ = layui.jquery;
    ...
    var quarterlyCycle = layui.quarterlyCycle;
    ...
    var obj = function () {
         ...
         * @param data
         */
        this.bindFormSubmit = function (formid, elements, type) {
            form.verify(this.objtable.config.form.customVerify);
            form.on('submit(' + formid + ')', function (data) {
                if (JSON.stringify(data.field) != "{}") {
                    ...
                    admin.req({
                        ...
                        url: self.objtable.config.url,
                        data: JSON.stringify(data.field),
                        done: function (result) {
                            layer.close(index);
                            if (result.code == 0) {//成功
                                layer.msg('操作成功！', {icon: 6, time: 500}, function () {
                                    if (self.objtable.dlgIndex) {
                                        layer.close(self.objtable.dlgIndex);
                                    }
                                    for (var i = 0; i < elements.length; i++) {
                                        // 表单有附件要单独提交
                                    ...
                                    table.reload(self.objtable.config.id);
                                });
                            } else {
         ...
         */
        this.createQueryForm = function (formid, withButton) {
            var elements = [];
            // 元素配置项
            var elementConfig = {};
            // 存储所有日期元素，最后统一渲染
            ...
            if (withButton == true) {
                ...
                $("#" + formid + " .layui-btn-reload").click(function () {
                    ...
                    var quarterlyCycleVal = quarterlyCycle.getComponentValue(formid);
                    if (quarterlyCycleVal != undefined) {
                        // 点击重置按钮  取消季度周期 quarterlyCycle 的查询值
                        if ($(this).attr("data-type") == "reset") {
                            formObject['quarter_value'] = "";
                        } else {
                            formObject['quarter_value'] = quarterlyCycleVal;
                        }
                    }
                    table.reload(self.objtable.config.id, {
                        page: {
            ...
            // 获取查询表单的元素
            for (var i = 0; i < this.objtable.config.form.queryCriteria.length; i++) {
                var queryElement = this.objtable.config.form.queryCriteria[i];
                if (typeof queryElement == 'number') {
                    elements.push(this.objtable.config.form.elements[queryElement])
                } else {
         ...
         */
        this.createForm = function (formid, elements, data, withSubmit, divId) {
            formid += "_form";
            ...
            this.bindFormSubmit(formid, this.objtable.config.form.elements, type);
        }
        /**
         * 生成layui表格
         * @param tableId 表格ID
         ...
         */
        this.createTable = function (tableId, tableConfig) {
            this.objtable = createTableComponent(tableId, tableConfig);
         ...
         * @param tableConfig 表格配置
         */
        this.createTree = function (treeId, treeSetting) {
            return treeComponent.initTree(treeId, treeSetting);
        };
        /**
         * 初始化表单配置
         * @param this.objtable 没有表格时单独配置表单项 url,form必需有
         */
        this.initForm = function (formConfig) {
            this.objtable.config = formConfig;
        };
        /**
         * 根据配置获取生成表单内容
         * @param data 表单数据
         ...
         * @param isEdit 是否可编辑
         */
        this.getForm = function (divId, data, withSubmit, isEdit) {
            var formId = this.objtable.config.id + "_form";
            // 生成form表单
            if (isEdit == true) {
                this.createForm(this.objtable.config.id, this.objtable.config.form.elements, data, withSubmit, divId);
            } else {
                this.createForm(this.objtable.config.id, this.objtable.config.form.elements, data, false, divId);
                formDisabled(formId);
            }
            return formId;
        };
         ...
         */
        this.showEditForm = function (data, callback) {
            var dlgSet = this.objtable.config.form.dlgSet;
            var title = "修改";
            if (data == null || data == "") {
                title = "新增";
            }
            ...
            this.objtable.dlgIndex = layerIndex;
            // 生成form表单
            this.createForm(this.objtable.config.id, this.objtable.config.form.elements, data, true, divId);
            // 调用回调方法
            if (callback != undefined) {
                var formId = this.objtable.config.id + "_form";
                callback(formId, data,layerIndex);
            }
         ...
         * 提供的提交表单接口
         */
        this.formSubmit = function (callBack) {
            var form = this.objtable.config.id + "_form";
         ...
         */
        this.showViewForm = function (data, callback) {
            var dlgset = this.objtable.config.form.dlgSet;
            var title = "查看";
            var dlgwidth = "70%";
            var dlgheight = "85%";
         ...
         * @param data 选中行数据
         */
        this.showDeleteDlg = function (data, callback) {
                ...
                admin.req({//根据
                    ...
                    , done: function (result) {
                        if (result.code == 0) {//成功
                            ...
                            table.reload(this.objtable.config.id); //刷新表格
                            if (callback) {
                                callback(data);
                            }
                        } else {
         ...
         * @param withSubmit
         */
        this.getQueryForm = function (divId, withButton, callback) {
            // 解决刚登陆没参数加载方法报错
            if (this.objtable.config.id == undefined) {
                return;
            }

            var formid = this.objtable.config.id + "search_form";
            var form = '<form style="display: inline;" onSubmit="return false;" class="layui-form" id="' + formid + '" lay-filter="' + formid + '">';

            $("#" + divId).append($(form));

            this.createQueryForm(formid, withButton);
            //回调
            if (callback != undefined) {
                callback(formid);
            }
         ...
         * @param formId
         */
        this.getQueryCriteria = function (formId) {
    ...
    exports('pageScaffoldingObj', obj);
})