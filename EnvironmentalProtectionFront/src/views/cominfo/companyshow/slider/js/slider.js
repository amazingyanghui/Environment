var slider = {
    func: function ($) {
        ...
        $('#sideCatalog-catalog dl').delegate('dd', 'click', function () {
            var choosed = $(this)[0].innerText;
            // if(choosed.indexOf("（无）") === -1){
                ...
                var windowTop = $('a[name="' + ddId + '"]').offset().top - 60;
                $('body,html').animate({scrollTop: windowTop}, 'fast');
                // var ddInew = ddId.split("-")[0] + "-";
                // $(".sideCatalog-item2").hide();
                // $(".sideCatalog-item2").each(function () {
                //     if($(this).attr("id").indexOf(ddInew) !== -1){
                //         $(this).show();
                //     }
                // })
            // }
        ...
        $(window).scroll(function () {
            ...
            for (var i=headLen-1; i>=0; i--) {
                if ($(this).scrollTop() >=(allEle.eq(i).offset().top -60) - allEle.eq(i).height()) {
                    // debugger
                    var index = i;
        ...
        function scrollSlide(that, index){
            // debugger
            var ddIndex = that.find('a').stop().attr('href').lastIndexOf('#');
            var ddId = that.find('a').stop().attr('href').substring(ddIndex+1);
            var ddInew = ddId.split("-")[0] + "-";
            $(".sideCatalog-item2").hide();
            $(".sideCatalog-item2").each(function () {
                if($(this).attr("id").indexOf(ddInew) !== -1){
    ...
    ,
    getParamByNameFromUrl: function (paramName) {
        var args = new Object();
